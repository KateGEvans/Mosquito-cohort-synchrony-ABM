...
to eat-or-not  ;; turtle procedure
  ifelse  age < 20 [
  set metabolism (metabolism-rate * mass)
  let food-acquired (energy + (0.5 * food * (1 - mass-to-energy-ratio)) - metabolism ) ;early instars can only acquire half the food available in a patch;
  let food-not-acquired (energy - metabolism)
  ifelse food > 0
  [ set mass mass + (0.5 * mass-to-energy-ratio * food ) ; 0.5 b/c only half the food in a patch is acquired by early larvae
  ...
  [
  set metabolism (metabolism-rate * mass)
  let food-acquired (energy + (food  * (1 - mass-to-energy-ratio)) - metabolism ) ; how much food goes to energy or mass determned by mass-to-energy-ratio
  let food-not-acquired (energy - metabolism)
  ifelse food > 0
  [ set mass mass + (mass-to-energy-ratio * food )
    set food 0
    set energy food-acquired
    stop ]
  [ set energy food-not-acquired
...
to mature
  ...
  if life-stage = 2 and mass >= eclosion-mass [set egg false set life-stage 3 set size 1 set shape "mosquito 2" move-to one-of air-patches set can-lay-eggs true set JM1 101 set JM2 101
    ...
    if (eclosion-time > 350 and eclosion-time < 490 ) [ set premort premort + 1 ]
    if (eclosion-time > 630 and eclosion-time < 770 ) [ set postmort postmort + 1]
  ]
end
...
to-report premort-adults
  report premort
end

to-report postmort-adults
  report postmort
end

;to-report avg-adult-energy
 ; let aae []
  ;ifelse turtles with [life-stage = 3] = 0
   ; [set aae 0]
  ;  [set aae (mean [energy] of turtles with [life-stage = 3])]
 ; report aae
;end

;to-report avg-eggs-produced
  ;let aep []
  ;ifelse turtles with [life-stage = 3] = 0
   ; [set aep 0]
  ;  [set aep (mean [eggs-produced] of turtles with [life-stage = 3])]
 ; report aep
;end

;to-report median-instar-age
  ;let mia []
  ;ifelse turtles with [life-stage = 2] = 0
   ; [set mia 0]
  ;  [set mia (median [instar-age] of turtles with [life-stage = 2])]
 ; report mia
;end

;to-report median-eclosion-age
 ; let mea []
 ;ifelse turtles with [life-stage = 3] = 0
 ;   [set mea 0]
 ;   [set mea (median [eclosion-age] of turtles with [life-stage = 3])]
 ; report mea
;end
...
<experiments>
  ...
  <experiment name="test_synch cycles" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <exitCondition>total-mosquito-number = 0</exitCondition>
