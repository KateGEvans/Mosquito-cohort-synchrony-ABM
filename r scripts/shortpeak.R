...
early_model.df <- read_csv("Data/Netlogo/general/cycles/shortpeak_early.csv") %>%
  clean_names()
...
late_model.df <- read_csv("Data/Netlogo/general/cycles/shortpeak_late.csv")%>%
  clean_names()
...
write_csv(late_subset.df, "data/Netlogo/general/cycles/intermediates/late_subset_shortpeak.csv")
...
write_csv(early_subset.df, "data/Netlogo/general/cycles/intermediates/early_subset_shortpeak.csv")




late_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/late_subset_shortpeak.csv") %>%
  clean_names()

early_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/early_subset_shortpeak.csv") %>%
  clean_names()


combined <- bind_rows(late_subset.df, early_subset.df)

write_csv(combined, "data/Netlogo/general/cycles/intermediates/combined_shortpeak.csv")
...
write_csv(forgraphs.df, "data/Netlogo/general/cycles/forgraphs/forgraphs_shortpeak.csv")
...
ggplot(forgraphs.df, aes(mort_percent, diff, color = hatch_interval)) + 
  ...
  theme(plot.subtitle = element_text(vjust = 1), 
                                                         ...
                                                         panel.background = element_rect(fill = "white")) +labs(title = "Comparing Adult Numbers Pre and Post Mortality, short peak",
...
write_csv(forsas, "data/Netlogo/general/sasfiles/forsas_shortpeak.csv")
