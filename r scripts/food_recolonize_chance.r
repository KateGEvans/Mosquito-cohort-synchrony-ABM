library(tidyverse)
library(lubridate)
library(scales)
library(patchwork)
library(readxl)
library(janitor)


early_model.df <- read_csv("data/Netlogo/general/cycles/13_food_recolonize_early.csv") %>%
  clean_names()

early_model.df <- early_model.df %>%
  mutate(timesteps = 0:1000) 


early_model_long.df <- early_model.df %>%
  gather(output_names, value, -timesteps)

# removes number to trial column then deletes numbers in names and then deletes last _ if there
early_model_long.df  <- early_model_long.df %>%
  mutate(trial = str_extract(output_names, "(\\d)+"),
         output_names = str_remove(output_names, "(\\d)+"),
         output_names = str_remove(output_names, "\\_$"))

# makes numeric
early_model_long.df  <- early_model_long.df %>%
  mutate(trial = as.numeric(trial))

# adds in 0 for NA values
early_model_long.df  <- early_model_long.df %>%  
  mutate(trial = ifelse(is.na(trial), 0, trial))


# this converts to wide
early_model_wide.df <- early_model_long.df %>%
  spread(key = output_names, value = value) 


# now Kate wants - starts with pre
early_subset.df <- early_model_wide.df %>%
  select(timesteps, trial, starts_with("pre"),
         starts_with("post"),
         starts_with("hatch"),
         starts_with("early"),
         starts_with("late"),
         starts_with("adult"),
         starts_with("total"),
         starts_with("food_recolonize")
  )


early_subset.df <- early_subset.df %>%
  gather(mort_time, mort_percent, -timesteps, -trial, - adult_number, -premort_adults, - postmort_adults,
         -hatch_interval, - total_mosquito_number, -food_recolonize_chance
  )

early_subset.df <- early_subset.df %>%
  mutate(mort_time = as.character(mort_time)) %>%
  mutate(hatch_interval = as.character(hatch_interval)) %>%
  mutate(food_recolonize_chance = as.character(food_recolonize_chance))


early_subset.df <- early_subset.df %>%
  mutate(mort_time  = recode(mort_time, "early_juvenile_mortality" = "early")) %>%
  mutate(mort_time  = recode(mort_time, "late_juvenile_mortality" = "late")) %>%
  mutate(hatch_interval = recode(hatch_interval, "1" = "asynch")) %>%
  mutate(hatch_interval  = recode(hatch_interval, "70" = "synch")) %>%
  mutate(food_recolonize_chance  = recode(food_recolonize_chance , "1" = "10%")) %>%
  mutate(food_recolonize_chance  = recode(food_recolonize_chance , "5" = "50%")) %>%
  mutate(food_recolonize_chance  = recode(food_recolonize_chance , "10" = "100%"))



early_only <- early_subset.df %>%
  filter(mort_time == "early") %>%
  filter(mort_percent > 0)


###late mortality files input ###


late_model.df <- read_csv("data/Netlogo/general/cycles/13_food_recolonize_late.csv") %>%
  clean_names()


late_model.df <- late_model.df %>%
  mutate(timesteps = 0:1000) 


late_model_long.df <- late_model.df %>%
  gather(output_names, value, -timesteps)

# removes number to trial column then deletes numbers in names and then deletes last _ if there
late_model_long.df  <- late_model_long.df %>%
  mutate(trial = str_extract(output_names, "(\\d)+"),
         output_names = str_remove(output_names, "(\\d)+"),
         output_names = str_remove(output_names, "\\_$"))

# makes numeric
late_model_long.df  <- late_model_long.df %>%
  mutate(trial = as.numeric(trial))

# adds in 0 for NA values
late_model_long.df  <- late_model_long.df %>%  
  mutate(trial = ifelse(is.na(trial), 0, trial))


# this converts to wide
late_model_wide.df <- late_model_long.df %>%
  spread(key = output_names, value = value)


# now Kate wants - starts with pre
late_subset.df <- late_model_wide.df %>%
  select(timesteps, trial, starts_with("pre"),
         starts_with("post"),
         starts_with("hatch"),
         starts_with("early"),
         starts_with("late"),
         starts_with("adult"),
         starts_with("total"),
         starts_with("food_recolonize")
  )


late_subset.df <- late_subset.df %>%
  gather(mort_time, mort_percent, -timesteps, -trial, - adult_number, -premort_adults, - postmort_adults,
         -hatch_interval, - total_mosquito_number, - food_recolonize_chance
  )

late_subset.df <- late_subset.df %>%
  mutate(mort_time = as.character(mort_time)) %>%
  mutate(hatch_interval = as.character(hatch_interval)) %>%
  mutate(food_recolonize_chance = as.character(food_recolonize_chance))

late_subset.df <- late_subset.df %>%
  mutate(mort_time  = recode(mort_time, "early_juvenile_mortality" = "early")) %>%
  mutate(mort_time  = recode(mort_time, "late_juvenile_mortality" = "late")) %>%
  mutate(hatch_interval = recode(hatch_interval, "1" = "asynch")) %>%
  mutate(hatch_interval  = recode(hatch_interval, "70" = "synch")) %>%
  mutate(food_recolonize_chance  = recode(food_recolonize_chance , "1" = "10%")) %>%
  mutate(food_recolonize_chance  = recode(food_recolonize_chance , "5" = "50%")) %>%
  mutate(food_recolonize_chance  = recode(food_recolonize_chance , "10" = "100%"))

late_subset.df <- late_subset.df %>%
  filter(mort_time == "late")


late_only <-late_subset.df %>%
  filter(mort_percent > 0 ) %>%
  mutate(trial = (trial + 1000))


control.df.synch <- late_subset.df  %>%
  filter(mort_percent == 0)  %>%
  filter(hatch_interval == "synch") %>% 
  arrange(trial) %>%
  mutate(trial = (trial = rep(0:74, each = 1001)))

control.df.asynch <- late_subset.df  %>%
  filter(mort_percent == 0)  %>%
  filter(hatch_interval == "asynch") %>% 
  arrange(trial) %>%
  mutate(trial = (trial = rep(75:149, each = 1001)))

control.df <- bind_rows(control.df.asynch, control.df.synch)


control.df.early <- control.df %>%
  mutate(mort_time  = recode(mort_time, "late"="early"))

late_subset.df <- bind_rows(late_only, control.df)

  
write_csv(late_subset.df, "data/Netlogo/general/cycles/intermediates/13_late_subset_food_recolonize.csv")


#incorporate the control groups into the early subset too
early_only <- early_only %>%
  mutate(trial = (trial + 150))

early_subset.df <- bind_rows(early_only, control.df.early)

write_csv(early_subset.df, "data/Netlogo/general/cycles/intermediates/13_early_subset_food_recolonize.csv")


#read in clean files
late_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/13_late_subset_food_recolonize.csv") %>%
  clean_names()

early_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/13_early_subset_food_recolonize.csv") %>%
  clean_names()


combined <- bind_rows(late_subset.df, early_subset.df)

write_csv(combined, "data/Netlogo/general/cycles/intermediates/13_combined_food_recolonize.csv")



#how many trials ended before 1000 timesteps?
end_point.df <- combined %>%
  group_by(trial) %>%
  filter(total_mosquito_number == 0) %>%
  filter(timesteps == min(timesteps))



#select the end time point for each trial when mosquitoes were still present
final_results.df <- combined %>% 
  group_by(trial) %>%
  filter(timesteps == max(timesteps))


#make a variable for the differnce between pre and post adults, and a variable for if the population persisted
forgraphs.frc.df <- final_results.df %>%
  mutate(diff = (postmort_adults - premort_adults)) %>%
  mutate(pop_persist = ifelse(timesteps < 1000, 0, 1))

write_csv(forgraphs.frc.df, "data/Netlogo/general/cycles/forgraphs/13_forgraphs_food_recolonize.csv")

forgraphs.frc.df <- read_csv("data/Netlogo/general/cycles/forgraphs/13_forgraphss_food_recolonize.csv") %>%
  clean_names()

#bring in control original group experiments
forgraphs.control.df <- read_csv("data/Netlogo/general/forgraphs_gen_cycles.csv") %>%
  clean_names()

forgraphs.control.df <-forgraphs.control.df %>%
  mutate(food_recolonize_chance = "20%")
  

#bind original experiments and food recolonize SA
forgraphs.df <- bind_rows(forgraphs.frc.df, forgraphs.control.df)

#make food_recolonize_chance compatible between data frames  
forgraphs.df <- forgraphs.df %>%
  mutate(food_recolonize_chance = as.character(food_recolonize_chance))

#manually order the food recolonize chance
ord_recolonize <- c( "10%", "20%", "50%", "100%")

forgraphs.df$food_recolonize_chance <- factor(forgraphs.df$food_recolonize_chance, levels = ord_recolonize)


write_csv(forgraphs.df, "data/Netlogo/general/cycles/forgraphs/13_forgraphs_food_recolonize_wcontrol.csv")



#color palette for graphs
cbPalette2 <- c("#56B4E9", "#009E73")


#summarize to help us find where the horizontal lines should go on the graph  
sum_forgraphs <- forgraphs.df %>%
  group_by(mort_percent, mort_time, hatch_interval, food_recolonize_chance) %>%
  summarize(mean_diff = mean(diff, na.rm = TRUE), min = min(diff), max = max(diff)) 

sum_forgraphs

forgraphs.df %>%
  ungroup()


#graph the food recolonize chance SA against original experiments
food_recolonize_plot_13 <- ggplot(forgraphs.df, aes(mort_percent, mean_diff, color = hatch_interval, shape = food_recolonize_chance)) + 
  geom_pointrange(mapping = aes(x = mort_percent, y = diff, color = hatch_interval),
                  stat = "summary",
                  fun.ymin = min,
                  fun.ymax = max,
                  fun.y = mean,
                  position = position_dodge(width = 5)) +
  scale_color_manual(values = cbPalette2)+
  facet_wrap(~mort_time)+
  geom_hline(yintercept= 0) +
  theme(plot.subtitle = element_text(vjust = 1), 
                                                         plot.caption = element_text(vjust = 1), 
                                                         axis.line = element_line(linetype = "solid"), 
                                                         panel.background = element_rect(fill = "white")) +labs(title = "Comparing Adult Numbers Pre and Post Mortality, Food Recolonization ", 
                                                                                                                x = "Mortality %", y = "Difference in Adult Number", 
                                                                                                                colour = "Cohort Synchony")+labs(shape = "Food Recolonization Chance")

food_recolonize_plot_13



ggsave("graphs/Netlogo/food_recolonize_plot_13.pdf")

###now for making a data set for SAS###

#get rid of control doubles (which we needed for graphing)
forsas <- forgraphs.frc.df %>%
  group_by(trial) %>%
  filter(!(mort_time == "early" & mort_percent == 0))

#label control
forsas <- forsas %>%
  mutate(mort_time  = if_else(mort_percent == 0, "control", mort_time)) 


#bring in the original experiments to compare later in SAS
forsas.control.df <- read_csv("data/Netlogo/general/sasfiles/forsas_2peaks.csv")  %>%
  clean_names()

forsas.control.df <- forsas.control.df %>%
  mutate(food_recolonize_chance = "20%")


#combine the food-r experiments and the original experiments
forsas_combined <- bind_rows(forsas, forsas.control.df)


write_csv(forsas_combined, "data/Netlogo/general/sasfiles/13_forsas_food_recolonize.csv")




