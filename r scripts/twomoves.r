...
early_model.df <- read_csv("data/Netlogo/general/cycles/13_2moves_early.csv") %>%
  clean_names()
...
early_model_wide.df <- early_model_wide.df %>%
  mutate(moves = "2")
...
late_model.df <- read_csv("data/Netlogo/general/cycles/13_2moves_late.csv") %>%
  clean_names()
...
late_model_wide.df <- late_model_wide.df %>%
  mutate(moves = "2")
...
write_csv(late_subset.df, "data/Netlogo/general/cycles/intermediates/13_late_subset_moves.csv")
...
write_csv(early_subset.df, "data/Netlogo/general/cycles/intermediates/13_early_subset_2moves.csv")



late_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/13_late_subset_moves.csv") %>%
  clean_names()

early_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/13_early_subset_2moves.csv") %>%
  clean_names()


combined <- bind_rows(late_subset.df, early_subset.df)

write_csv(combined, "data/Netlogo/general/cycles/intermediates/13_combined_moves.csv")
...
forgraphs.move.df <- final_results.df %>%
  mutate(diff = (postmort_adults - premort_adults)) %>%
  mutate(pop_persist = ifelse(timesteps < 1000, 0, 1))

write_csv(forgraphs.move.df, "data/Netlogo/general/cycles/forgraphs/13_forgraphs_moves.csv")

forgraphs.move.df <- read_csv("data/Netlogo/general/cycles/forgraphs/13_forgraphs_moves.csv") %>%
  clean_names()
...
forgraphs.move.df <- forgraphs.move.df %>%
  mutate(moves = as.character(moves))

#bind original experiments and moves SA
forgraphs.df <- bind_rows(forgraphs.move.df, forgraphs.control.df)
  
forgraphs.df <- forgraphs.df %>%
  mutate(moves = as.character(moves))


write_csv(forgraphs.df, "data/Netlogo/general/cycles/forgraphs/13_forgraphs_moves_wcontrol.csv")
...
sum_forgraphs <- forgraphs.df %>%
  group_by(mort_percent, mort_time, hatch_interval, moves) %>%
...
twomoves_plot_13 <- ggplot(forgraphs.df, aes(mort_percent, diff, color = hatch_interval, shape = moves)) + 
  ...
  theme(plot.subtitle = element_text(vjust = 1), 
                                                         ...
                                                         panel.background = element_rect(fill = "white")) +labs(title = "Comparing Adult Numbers Pre and Post Mortality, Movement Stochasticity ", 
                                                                                                                x = "Mortality %", y = "Difference in Adult Number", 
                                                                                                                colour = "Cohort Synchony")+labs(shape = "Number of Moves")


twomoves_plot_13  

ggsave("graphs/Netlogo/twomoves_plot_13.pdf")

###now for making a data set for SAS###

#get rid of control doubles (which we needed for graphing)
forsas <- forgraphs.move.df %>%
  group_by(trial) %>%
...
write_csv(forsas_combined, "data/Netlogo/general/sasfiles/13_forsas_moves.csv")
