...
early_model.df <- read_csv("data/Netlogo/general/cycles/13_stochastic_e_early.csv") %>%
  clean_names()
...
late_model.df <- read_csv("data/Netlogo/general/cycles/13_stochastic_e_late.csv") %>%
  clean_names()
...
write_csv(late_subset.df, "data/Netlogo/general/cycles/intermediates/13_late_subset_stochastic_e.csv")
...
final_early_results.df <- early_subset.df %>% 
  group_by(trial) %>%
  filter(timesteps == max(timesteps))

final_late_results.df <- late_subset.df %>% 
  group_by(trial) %>%
  filter(timesteps == max(timesteps))

write_csv(early_subset.df, "data/Netlogo/general/cycles/intermediates/13_early_subset_stochastic_e.csv")


late_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/13_late_subset_stochastic_e.csv") %>%
  clean_names()

early_subset.df <- read_csv("data/Netlogo/general/cycles/intermediates/13_early_subset_stochastic_e.csv") %>%
  clean_names()


combined <- bind_rows(late_subset.df, early_subset.df)

write_csv(combined, "data/Netlogo/general/cycles/intermediates/13_combined_stochastic_e.csv")
...
forgraphs.energy.df <- final_results.df %>%
  mutate(diff = (postmort_adults - premort_adults)) %>%
  mutate(pop_persist = ifelse(timesteps < 1000, 0, 1))

write_csv(forgraphs.energy.df, "data/Netlogo/general/cycles/forgraphs/13_forgraphs_energy.csv")

forgraphs.energy.df <- read_csv("data/Netlogo/general/cycles/forgraphs/13_forgraphs_energy.csv") %>%
  clean_names()
...
forgraphs.control.df <- forgraphs.control.df %>%
  mutate(e_stderr = "uniform")


forgraphs.energy.df <- forgraphs.energy.df  %>%
  mutate(e_stderr = as.character(e_stderr))

#bind original experiments and moves SA
forgraphs.df <- bind_rows(forgraphs.energy.df, forgraphs.control.df)

forgraphs.df <- forgraphs.df  %>%
  mutate(e_stderr = as.character(e_stderr))


write_csv(forgraphs.df, "data/Netlogo/general/cycles/forgraphs/13_forgraphs_energy_wcontrol.csv")
...
sum_forgraphs<- forgraphs.df %>%
  group_by(mort_percent, mort_time, hatch_interval, e_stderr) %>%
...
stochastic_e__plot_13 <- ggplot(forgraphs.df, aes(mort_percent, diff, color = hatch_interval, shape = e_stderr)) + 
  ...
  theme(plot.subtitle = element_text(vjust = 1), 
                                                         ...
                                                         panel.background = element_rect(fill = "white")) +labs(title = "Comparing Adult Numbers Pre and Post Mortality, Initial Energy SD ", 
                                                                                                                x = "Mortality %", y = "Difference in Adult Number", 
                                                                                                                colour = "Cohort Synchony")+labs(shape = "Energy Standard Deviation")


stochastic_e__plot_13 

ggsave("graphs/Netlogo/stochastic_e__plot_13 .pdf")

###now for making a data set for SAS###

#get rid of control doubles (which we needed for graphing)
forsas <- forgraphs.energy.df %>%
  group_by(trial) %>%
...
write_csv(forsas_combined, "data/Netlogo/general/sasfiles/13_forsas_energy.csv")
